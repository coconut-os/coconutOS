/*
 * llama-pipeline — pipeline-parallel transformer inference across two shards.
 *
 * Same model and math as llama-inference, but the 2-layer transformer is split
 * across 2 shards using IPC channels. Stage 0 runs layer 0, stage 1 runs
 * layer 1. Activations pass between stages via channel_send/recv.
 *
 * Stage determined at runtime from a kernel-provided config page at VA 0x10000.
 * Both shards load the full model (all weight pointers) but only access their
 * assigned layers during forward.
 *
 * Output tokens must match llama-inference exactly — same model, same math.
 */
...
/* Forward pass for a subset of layers. KV cache uses local layer offset
 * (l - layer_start) so each stage has its own independent cache. */
static void forward_layers(struct config *cfg, struct weights *w,
                           struct run_state *s, int pos,
                           int layer_start, int layer_end)
{
    ...
    int local_layers = layer_end - layer_start;

    for (int l = layer_start; l < layer_end; l++) {
        int ll = l - layer_start; /* local layer index for KV cache */
        ...
        /* Cache K and V — local layer offset */
        int loff = ll * cfg->seq_len * kv_dim;
...
int main(void)
{
    /* Read pipeline config page */
    const struct pipeline_config *pcfg = coconut_pipeline_config();
    if (!pcfg) {
        coconut_puts("llama-pipeline: bad config magic\n");
        return 1;
    }

    uint32_t stage = pcfg->stage;
    uint32_t channel_id = pcfg->channel_id;
    int layer_start = (int)pcfg->layers_start;
    int layer_end = (int)pcfg->layers_end;

    coconut_puts("llama-pipeline[");
    print_u64(stage);
    coconut_puts("]: starting (layers ");
    print_u64(layer_start);
    coconut_puts("-");
    print_u64(layer_end);
    coconut_puts(")\n");
    ...
    if (coconut_mmap(HEAP_VA, HEAP_PAGES) != 0) {
        coconut_puts("llama-pipeline: mmap failed\n");
        return 1;
    }
    struct heap h;
    ...
    if (fd == COCONUT_ERROR) {
        coconut_puts("llama-pipeline: failed to open /model.bin\n");
        return 1;
    }

    uint64_t file_size = coconut_fs_stat(fd);
    if (file_size == COCONUT_ERROR || file_size == 0) {
        coconut_puts("llama-pipeline: failed to stat model\n");
        return 1;
    }

    uint8_t *model_data = (uint8_t *)bump_alloc(&h, file_size);
    if (!model_data) {
        coconut_puts("llama-pipeline: heap OOM for model\n");
        return 1;
    }
    ...
    if (total_read < 28) {
        coconut_puts("llama-pipeline: model file too small\n");
        return 1;
    }
    ...
    /* 5. Allocate run state — KV cache sized for local layers only */
    int local_layers = layer_end - layer_start;
    struct run_state s;
    ...
    /* KV cache only needs local_layers worth of storage */
    s.key_cache   = (float *)bump_alloc(&h, local_layers * cfg.seq_len * kv_dim * 4);
    s.value_cache = (float *)bump_alloc(&h, local_layers * cfg.seq_len * kv_dim * 4);

    if (!s.x || !s.value_cache) {
        coconut_puts("llama-pipeline: heap OOM for run state\n");
        return 1;
    }

    /* 6. Pipeline inference loop — 16 tokens */
    int n_tokens = 16;
    int dim = cfg.dim;

    if (stage == 0) {
        /* Stage 0: embed token -> layer 0 -> send activation -> recv next token */
        int token = 0; /* start token */

        for (int pos = 0; pos < n_tokens; pos++) {
            /* Embed token into s.x */
            float *embed = w.token_embedding_table + token * dim;
            for (int i = 0; i < dim; i++)
                s.x[i] = embed[i];

            /* Forward through this stage's layers */
            forward_layers(&cfg, &w, &s, pos, layer_start, layer_end);

            /* Send activation vector (dim floats) to stage 1 */
            coconut_channel_send(channel_id, s.x, dim * 4);

            /* Receive next token from stage 1 */
            uint32_t next_token = 0;
            coconut_channel_recv(channel_id, &next_token, 4);

            coconut_puts("llama-pipeline[0]: token ");
            print_u64(pos);
            coconut_puts(" -> '");
            char ch = (next_token < 32) ? VOCAB[next_token] : '?';
        ...
        coconut_puts("llama-pipeline[0]: pipeline complete (16 tokens)\n");

    } else {
        /* Stage 1: recv activation -> layer 1 -> final RMSNorm + classify -> send token */

        for (int pos = 0; pos < n_tokens; pos++) {
            /* Receive activation vector from stage 0 */
            coconut_channel_recv(channel_id, s.x, dim * 4);
            ...
            uint32_t next_token = (uint32_t)next;

            /* Send next token back to stage 0 */
            coconut_channel_send(channel_id, &next_token, 4);

            coconut_puts("llama-pipeline[1]: token ");
            print_u64(pos);
            coconut_puts(" -> '");
            char ch = (next < 32) ? VOCAB[next] : '?';
            coconut_serial_write(&ch, 1);
            coconut_puts("'\n");
        }

        coconut_puts("llama-pipeline[1]: pipeline complete (16 tokens)\n");
    }
